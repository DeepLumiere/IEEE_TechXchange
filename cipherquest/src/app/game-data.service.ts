...
export interface Mission {
  ...
  description: string;
  type: string;
  storyIntro?: string; // Dystopian story prompt line
}
...
export class GameDataService {
  ...
  readonly authProfile = signal<AuthProfile | null>(null);

  readonly operatorName = signal<string>(localStorage.getItem('operatorName') || '');
  readonly selectedAvatar = signal<AvatarVariant>(
      AVATAR_VARIANTS.find(v => v.id === localStorage.getItem('avatarId')) || AVATAR_VARIANTS[0]
  );
  ...
  readonly chapters = signal<Chapter[]>([
    ...
    },
    {
      ...
      missions: [
        { id: '4-1', name: 'DNA Sequencing Hack', description: '(a,b) becomes (a+b,a*b)', type: 'pairing', storyIntro: 'SysAdmin: Syndicate genetics labs use nonlinear mapping of adjacent char values. We must separate the biological pairs!' },
  ...
  readonly completedMissions = signal<Set<string>>(new Set<string>());
  readonly missionStats = signal<Record<string, { attempts: number, timeMs: number }>>({});
  
  readonly justCompletedMissionId = signal<string | null>(null);
  readonly justCompletedChapterId = signal<number | null>(null);
  
  readonly earnedBadges = signal<Badge[]>([]);

  readonly totalMissions = computed(() => {
    return this.chapters().reduce((acc, curr) => acc + curr.missions.length, 0);
  });

  readonly progressPercentage = computed(() => {
    return Math.round((this.completedMissions().size / this.totalMissions()) * 100) || 0;
  });
  
  readonly isQuantumAscentUnlocked = computed(() => {
      // Unlocked when all missions in chapters 1-4 are done
      return this.completedMissions().size === this.totalMissions();
  });

  readonly isCipherMaster = this.isQuantumAscentUnlocked;
  ...
  async checkAuthStatus(): Promise<boolean> {
    try {
      ...
      if (data.authenticated && data.profile) {
        ...
        // Don't sync firebase username to operatorName - it's a local-only field
        // used for story progress and ADMIN cheatcode
        return true;
      }
      this.isAuthenticated.set(false);
      this.authProfile.set(null);
  ...
  async logout() {
    ...
    this.completedMissions.set(new Set());
    this.missionStats.set({});
  ...
  loadProgress() {
    ...
    if (op.toUpperCase() === 'ADMIN') {
        const completed = new Set<string>();
        const stats: Record<string, { attempts: number, timeMs: number }> = {};
        this.chapters().forEach(chap => {
            chap.missions.forEach(m => {
                completed.add(m.id);
                stats[m.id] = { attempts: 1, timeMs: 420 }; // default perfect stats
            });
        });
        this.completedMissions.set(completed);
        this.missionStats.set(stats);
        this.checkBadges();
        return;
    }

    this.http.get<{progress: {missionId: string, attempts: number, timeMs: number}[]}>(`/api/progress?operatorName=${encodeURIComponent(op)}`).subscribe((res) => {
      if (res.progress) {
          const completed = new Set<string>();
          const stats: Record<string, { attempts: number, timeMs: number }> = {};
          res.progress.forEach(p => {
              completed.add(p.missionId);
              stats[p.missionId] = { attempts: p.attempts, timeMs: p.timeMs };
          });
          this.completedMissions.set(completed);
          this.missionStats.set(stats);
          this.checkBadges();
  ...
  saveMissionProgress(missionId: string, attempts: number, timeMs: number) {
    const op = this.operatorName();
    if (op) {
      this.http.post('/api/progress', { operatorName: op, missionId, attempts, timeMs }).subscribe();
    }
    
    // Update local state
    this.missionStats.update(s => ({ ...s, [missionId]: { attempts, timeMs } }));
    this.completedMissions.update(set => {
      const newSet = new Set(set);
      newSet.add(missionId);
      return newSet;
    });
    this.justCompletedMissionId.set(missionId);
    this.checkBadges();
  }
  ...
  async setOperatorName(name: string): Promise<void> {
    ...
    if (this.isAuthenticated() && upper !== 'ADMIN') {
        try {
            await this.http.patch('/api/auth/profile', { username: upper }, { withCredentials: true }).toPromise();
            this.authProfile.update(p => p ? { ...p, username: upper } : null);
        } catch (e: any) {
            throw new Error(e.error?.error || 'Failed to update operator name.');
        }
    }
    
    this.operatorName.set(upper);
    localStorage.setItem('operatorName', upper);
    this.loadProgress();
  }

  setAvatar(avatar: AvatarVariant) {
      this.selectedAvatar.set(avatar);
      localStorage.setItem('avatarId', avatar.id);
  ...
  clearJustCompleted() {
    this.justCompletedMissionId.set(null);
    this.justCompletedChapterId.set(null);
  }
  ...
  private checkBadges() {
      ...
      for (const chapter of chapters) {
          ...
          if (!newBadges.find(b => b.id === badgeId)) {
              const allCompleted = chapter.missions.every(m => completed.has(m.id));
              if (allCompleted) {
                  newBadges.push({
                      id: badgeId,
                      name: `Protocol ${chapter.id} Master`,
                      description: `Completed all missions in ${chapter.title}.`,
                      earnedAt: Date.now(),
                      icon: chapter.id === 1 ? 'vpn_key' : chapter.id === 2 ? 'lock_clock' : chapter.id === 3 ? 'calculate' : 'enhanced_encryption'
                  });
                  this.justCompletedChapterId.set(chapter.id);
                  updated = true;
              }
          }
      }
      ...
      if (this.isCipherMaster() && !newBadges.find(b => b.id === 'badge-master')) {
          newBadges.push({
              id: 'badge-master',
              name: 'CipherMaster',
              description: 'Completed all story mode sectors. You are a true operator.',
              earnedAt: Date.now(),
              icon: 'military_tech'
          });
          updated = true;
      ...
      if (updated) {
          this.earnedBadges.set(newBadges);
      }
  }
}
